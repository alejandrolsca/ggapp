with working_days as (
	select 
		count(*) 
	from generate_series($1,$2,interval '1 day') days
	where extract('ISODOW' from days) not in (6,7)
...
((select "count" from working_days)*8) as working_hours,
((print_runs/ma_velocity)/((select "count" from working_days)*8)) as usage_percentage
from (
	select
		printruns.ma_id,
		printruns.ma_name,
	...
	group by printruns.ma_id, printruns.ma_name, printruns.ma_velocity
