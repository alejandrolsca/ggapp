...
module.exports = (function (angular) {
    ...
    return ['$scope', 'shippingListFiscalInvoiceFac', '$location', 'i18nFilter', '$stateParams', '$filter', 'authService',
        function ($scope, shippingListFiscalInvoiceFac, $location, i18nFilter, $stateParams, $filter, authService) {
            ...
            $scope.exportXLS = function () {
                ...
                const fileName = `fiscal_invoice_${$stateParams.sl_id}_${timestamp}.xlsx`;
            ...
            $scope.$on('$viewContentLoaded', async () => {
                ...
                var rows = undefined;
                let sl_date
                shippingListFiscalInvoiceFac.getSL().then(function (promise) {
                    const { data } = promise
                    ...
                    shippingListFiscalInvoiceFac.searchWoId($scope.fmData.cl_id, $scope.fmData.wo_id, $scope.fmData.zo_id).then(function (promise) {
                        $scope.loading = false;
                        const { data } = promise
