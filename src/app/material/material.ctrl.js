module.exports = (function(angular){
    ...
    return ['$scope', 'materialFac', 'i18nFilter',
    function ($scope, materialFac, i18nFilter) {
        ...
        $scope.formatItem = function(s, e, cell) {
            ...
            if ((e.panel.cellType == wijmo.grid.CellType.Cell) && (e.col == 0)) {
                mt_id = e.panel.getCellData(e.row,1,false);
        ...
        $scope.initGrid = function(s, e) {
            for (var i = 0; i < $scope.columns.length; i++) {
                ...
                col.header = i18nFilter("material.labels." + $scope.columns[i].replace('_','-'));
                col.wordWrap = false;
                col.width = 150;
                s.columns.push(col);
            }
        ...
        $scope.$on('$viewContentLoaded', function () {
            ...
            $scope.loading = true;
            materialFac.data().then(function(promise){
                $scope.loading = false;
